...
function OutputMitigationToPs1 
{
    ...
    $cmt = "# This PowerShell script was generated as a mitigation by TLS 1.2 readiness checker."
    $lines = @($cmt) + $script
...
"@
        ...
        {
            Write-Detail "No need to change the GP override since cipher suites required by the server are already enabled."
        }
    }
    else
    {
        ...
        {
            Write-Detail "No Group Policy cipher suites override defined. No need to create the GP override since cipher suites required by the server are already enabled."
        }
    }
    ...
    function CheckKeyExchangeEnabled
    {
        ...
        {    
            ...
            else
            {
                ...
                {    
                    Write-nonOK "ISSUE FOUND: No TLS 1.2 cipher suites required by the server remain enabled after applying $name disablement."
    ...
    else
    {
        ...
        {
            Write-OK "Elliptic curve check passed: at least one elliptic curve supported by the server is enabled."
        ...
        else
        {
            ...
            $reasonMsg = "All elliptic curves supported by the server are disabled. This makes ECDHE cipher suites unusable for connection."
